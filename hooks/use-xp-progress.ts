 ...
 */
export function useXPProgress(playerUid?: string): UseXPProgressReturn {
  // Core state
   ...
   */
  const calculateLevelFromXP = useCallback(
    (
      totalXP: number,
    ): {
      level: number;
      ...
      while (totalXPRequired + xpForCurrentLevel <= totalXP) {
        ...
        xpForCurrentLevel = Math.floor(
          XP_CONSTANTS.XP_PER_LEVEL * 1.1 ** (level - 1),
        );
      }
  ...
  const calculateXPGain = useCallback(
    (gameResult: GameResult): XPCalculation => {
      const baseXP = XP_CONSTANTS.BASE_XP_PER_GAME;

      // Position bonus based on final placement
      const positionMultiplier =
        XP_CONSTANTS.POSITION_MULTIPLIERS[
          gameResult.position as keyof typeof XP_CONSTANTS.POSITION_MULTIPLIERS
        ] || 0.2;
      const positionBonus = Math.floor(baseXP * (positionMultiplier - 1));

      // Rounds bonus for rounds won
      const roundsBonus =
        gameResult.roundsWon * XP_CONSTANTS.ROUNDS_BONUS_PER_WIN;
  ...
  const calculateDerivedData = useCallback(
    (xpInfo: XPData) => {
      ...
      const totalXPForCurrentLevel =
        levelInfo.totalForNext - levelInfo.xpForNext;
      const progress =
   ...
   */
  const fetchPlayerXPData = useCallback(
    async (_uid: string): Promise<XPData> => {
  ...
  const simulateXPGain = useCallback(
    async (xpGain: number): Promise<void> => {
      if (!xpData) return;
